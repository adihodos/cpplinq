...
namespace cpplinq
{
    ...
    CPPLINQ_INLINEMETHOD detail::from_range<typename std::initializer_list<TValueType>::const_iterator> from(
        std::initializer_list<TValueType> init_list)
    {
        return detail::from_range<typename std::initializer_list<TValueType>::const_iterator>{
            std::begin(init_list),
            std::end(init_list)
        };
    }
    ...
    template <typename TContainer>
    CPPLINQ_INLINEMETHOD detail::from_copy_range<typename detail::cleanup_type<TContainer>::type>
    ...
    CPPLINQ_INLINEMETHOD detail::from_range<decltype(std::begin(std::declval<TValueType (&)[N]>()))>
    from(TValueType (&array_ref)[N])
    {
        using range_type = decltype(std::begin(std::declval<TValueType (&)[N]>()));
        return detail::from_range<range_type>{std::begin(array_ref), std::end(array_ref)};
    }
